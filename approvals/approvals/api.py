...
def get_approval_roles(doc: Document, method: str | None = None):
    ...
    if not roles:
        ...
        if not fallback_approver:
            frappe.throw(
                _(
                    "No approvers found. Please set a fallback approver role in Document Approval Settings."
                )
            )
...
def fetch_approvals_and_roles(doc: Document | str, method: str | None = None):
    ...
    for role in roles:
        ...
        assigned_user = (
            "You" if assignments.get(role, role) == frappe.session.user else assigned_user
        )
        approver = ""
...
def reject_document(doc: Document | str, role=None, comment: str = "", method: str | None = None):
    ...
    if comment:
        rejection = add_comment(
            doc.doctype, doc.name, comment, frappe.session.user, frappe.session.user
        )
