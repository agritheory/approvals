...
class DocumentApprovalRule(Document):
    ...
    def apply(
        ...
        result = frappe.safe_eval(
            self.condition, eval_globals=eval_globals, eval_locals=eval_locals
        )
    ...
    def assign_user(self, doc: Document):
        if doc.meta:
            ...
            if workflow_name:
                ...
                approval_state = frappe.get_cached_value(
                    "Workflow", workflow_name, "approval_state"
                )
        ...
        if self.primary_assignee:
            ...
            index = (
                users.index(self.last_user) if self.last_user and self.last_user in users else 0
            )
        ...
        if not frappe.db.get_value(
            ...
            todo.description = (
                self.get_message(doc)
                if self.message
                else frappe._("A document has been assigned to you")
            )
