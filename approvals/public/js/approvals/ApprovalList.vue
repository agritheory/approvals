...
<div>
	...
	>Add Approver <i class="octicon octicon-plus" style="margin-left: 2px; "></i></a>
	<br>
	<a 
		v-show="docstatus()"
		class="text-muted"
...
export default {
	...
	methods: {
		...
		handleAddApprover(){
			approvals.add_approver_dialog(cur_frm)
			.then(user => {
				cur_dialog.hide()
				frappe.xcall('approvals.approvals.api.add_user_approval', {doc: cur_frm.doc, user: user.user})
				.then((r) => {
					this.fetchApprovalsAndRoles()
				})
			})
		...
		handleRemoveApprover(){
			let userApprovals = this.approvals.filter(
				approval => { return approval.approval_role == 'User Approval' }
			).map(approval => approval.assigned_to_user)
			approvals.remove_approver_dialog(cur_frm, userApprovals)
			.then(user => {
				let username = ''
				if(user.user.includes("@")){
					username = user.user
				} else {
					username = this.approvals.filter(
						approval => { return approval.assigned_to_user == user.user }
					).map(approval => approval.assigned_username)[0]
				}
				cur_dialog.hide()
				frappe.xcall('approvals.approvals.api.remove_user_approval', {doc: cur_frm.doc, user: username})
				.then((r) => {
					this.fetchApprovalsAndRoles()
				})
			})
